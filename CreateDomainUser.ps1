...
function Office 
    {
            Write-Host "Creating Domain Account"
            $DomainPassword = ConvertTo-SecureString -String $script:Password -AsPlainText -Force
            New-ADUser -Name "$($FirstName) $($Lastname)" -GivenName $script:Firstname -Surname $script:Lastname -Department $script:Department -Office $script:Location -Title $Script:Title -AccountPassword $DomainPassword -UserPrincipalName $script:Username -ChangePasswordAtLogon $True -Enabled $True
            Write-Host "Starting sleep for AD Connect"
            Start-Sleep -Seconds 900
            Write-Host "Not Yet"
            Start-Sleep -Seconds 900
            $wshell = New-Object -ComObject Wscript.Shell
            $wshell.Popup("Get back to onboarding this user yo!!!",0,"Done",0x1)
